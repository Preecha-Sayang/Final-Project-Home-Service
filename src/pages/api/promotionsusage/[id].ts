    ...
    if (req.method === "PATCH") {
      ...
      if (code !== undefined) {
        if (!code || code.length > 64 || !/^[A-Za-z0-9_-]+$/.test(code)) {
          return res
            .status(400)
            .json({ ok: false, message: "Invalid code format." });
        }
      }
      ...
      if (dtype !== undefined && dtype !== "fixed" && dtype !== "percent") {
        return res
          .status(400)
          .json({ ok: false, message: "Invalid discount_type." });
      }
      ...
      if (dval !== undefined && !Number.isFinite(dval)) {
        return res
          .status(400)
          .json({ ok: false, message: "Invalid discount_value." });
      }
      if (dtype === "percent" && dval != null && (dval < 0 || dval > 100)) {
        return res
          .status(400)
          .json({ ok: false, message: "Percent must be 0-100" });
      }
      if (dtype === "fixed" && dval != null && dval < 0) {
        return res
          .status(400)
          .json({ ok: false, message: "Amount must be >= 0" });
      }
      ...
      if (body.usage_limit !== undefined) {
        ...
        if (ul != null && (!Number.isInteger(ul) || ul < 0)) {
          return res
            ...
            .json({
              ok: false,
              message: "usage_limit must be integer >= 0 or null",
            });
        }
      }
