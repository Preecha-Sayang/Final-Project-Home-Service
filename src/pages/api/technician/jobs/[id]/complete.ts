...
async function handler(req: AdminRequest, res: NextApiResponse) {
    ...
    try {
        ...
        if (row.status_id !== BookingStatusId.InProgress || row.admin_id !== adminId) {
            await query("ROLLBACK"); return res.status(409).json({ ok: false, message: "cannot complete" });
        }
        ...
        await query(
            "INSERT INTO booking_actions (booking_id, actor_admin_id, action, meta) VALUES ($1,$2,'TECH_COMPLETE','{}'::jsonb)",
            [bookingId, adminId]
        );
        await query("COMMIT");
