...
async function handler(req: AdminRequest, res: NextApiResponse) {
    ...
    try {
        ...
        if (row.admin_id !== adminId || row.status_id !== BookingStatusId.WaitingProcess) {
            await query("ROLLBACK");
            return res.status(409).json({ ok: false, message: "cannot complete" });
        }
        ...
        await query(
            `INSERT INTO booking_actions 
             ...
             VALUES ($1, $2, 'TECH_COMPLETE', '{}'::jsonb)`,
            [bookingId, adminId]
        );
