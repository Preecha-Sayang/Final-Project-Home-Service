...
async function handler(req: AdminRequest, res: NextApiResponse) {
    ...
    const meta = (req.body ?? {}) as { lat?: number; lng?: number };
    ...
    try {
        ...
        if (row.status_id !== BookingStatusId.WaitingProcess || row.admin_id !== adminId) {
            await query("ROLLBACK"); return res.status(409).json({ ok: false, message: "cannot start" });
        }
