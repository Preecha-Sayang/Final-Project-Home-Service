...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    try {
        if (req.method === "GET") {
            const rows = await sql/*sql*/`SELECT * FROM addresses ORDER BY address_id DESC LIMIT 100` as any[];
            return res.status(200).json({ ok: true, items: rows });
        }

        if (req.method === "POST") {
            const body = typeof req.body === "string" ? JSON.parse(req.body) : req.body || {};
            const { user_id = null, label = null, place_name = null, address_line1 = null, address_line2 = null,
                district = null, city = null, province = null, postal_code = null, country = null, lat, lng } = body;
