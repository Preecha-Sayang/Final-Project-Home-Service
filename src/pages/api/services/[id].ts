...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    if (req.method === "GET") {
        try {
            ...
            return res.status(500).json({ ok: false, message: e?.message || "Get failed" });
        }
    }
    ...
    if (req.method === "PATCH") {
        try {
            ...
            let image_url = old.image_url as string | null;
            let image_public_id = old.image_public_id as string | null;
            ...
            // ไม่มีไฟล์ใหม่ > อัปเดตฟิลด์ทั่วไป
            const rows = await sql/*sql*/`
                UPDATE services
                SET servicename = ${servicename},
                    category_id = ${category_id},
    ...
    if (req.method === "DELETE") {
        try {
            ...
            const oldPub = current[0]?.image_public_id as string | null;
            ...
            if (oldPub) { try { await cloudinary.uploader.destroy(oldPub); } catch { } }
