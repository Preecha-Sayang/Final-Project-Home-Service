import ButtonPrimary from "../button/buttonprimary";

interface BookingCardProps {
  itemCode: string;
  operationDateTime: string;
  employee: string;
  items: string[];
  status: string;
  totalPrice: string;
  onViewDetails?: () => Promise<void> | void;
}

function BookingCard({ 
  itemCode, 
  operationDateTime, 
  employee, 
  items, 
  status, 
  totalPrice, 
  onViewDetails 
}: BookingCardProps) {
  return (
    <div className="w-full max-w-[831px] bg-white rounded-lg border border-gray-300 p-4">
      {/* Header */}
      <div className="mb-3">
        <h2 className="text-lg font-bold text-gray-900">รหัสรายการ: {itemCode}</h2>
      </div>

      <div className="flex justify-between items-start">
        {/* Left Column - Details */}
        <div className="flex-1 pr-4">
          {/* Operation Date/Time */}
          <div className="flex items-center gap-2 text-sm text-gray-600 mb-2">
            <img src="/images/icon_tasklist.svg" alt="tag" width={20} height={20} />
            <span>วันเวลาดำเนินการ: {operationDateTime}</span>
          </div>

          {/* Employee */}
          <div className="flex items-center gap-2 text-sm text-gray-600 mb-2">
            <img src="/images/icon_user.svg" alt="tag" width={20} height={20} />
            <span>พนักงาน: {employee}</span>
          </div>

          {/* Items */}
          <div className="mb-2">
            <span className="text-sm font-medium text-gray-700">รายการ:</span>
            <ul className="mt-1 space-y-1">
              {items.map((item, index) => (
                <li key={index} className="text-sm text-gray-900 flex items-center gap-2">
                  <span className="w-1.5 h-1.5 bg-gray-400 rounded-full"></span>
                  {item}
                </li>
              ))}
            </ul>
          </div>
        </div>

        {/* Right Column - Status and Price */}
        <div className="flex flex-col items-end gap-2">
          {/* Status */}
          <div className="flex items-center gap-2 text-sm">
            <span className="font-medium text-gray-700">สถานะ:</span>
            <span className={`px-2 py-1 rounded-full text-sm ${
                status === "รอดำเนินการ" ? "bg-gray-200 text-gray-800" :
                status === "กำลังดำเนินการ" ? "bg-yellow-100 text-yellow-900" :
                status === "ดำเนินการสำเร็จ" ? "bg-green-100 text-green-900" :
              "bg-gray-200 text-gray-600"
            }`}>
              {status}
            </span>
          </div>

          {/* Total Price */}
          <div className="text-right text-sm font-medium text-gray-700">
            ราคารวม: {totalPrice} ฿
          </div>
        </div>
      </div>

      {/* Action Button */}
      <div className="flex justify-end">
        <ButtonPrimary onClick={onViewDetails}>
          ดูรายละเอียด
        </ButtonPrimary>
      </div>
    </div>
  );
}

export default BookingCard;
